...
plot(window(co2, start = c(2000,1)), 
     main = "Carbon Dioxide Levels with Monthly Symbols", 
     ylab = "CO2")
...
plot(y = ARMAacf(ma = c(0.5,rep(0,10),0.8,0.4), lag.max = 13)[-1], 
     x = 1:13, type = "h", xlab = "Lag k", ylab = expression(rho[k]), 
     axes = FALSE, ylim = c(0,0.6))
points(y = ARMAacf(ma = c(0.5,rep(0,10),0.8,0.4),lag.max=13)[-1],
       x = 1:13, pch = 20)
abline(h = 0)
axis(1, at = 1:13, labels = c(1,NA,3,NA,5,NA,7,NA,9,NA,11,NA,13))
axis(2)
text(x = 7, y = 0.5, labels = expression(list(theta==-0.5,Theta==-0.8)))

plot(y = ARMAacf(ma=c(-0.5,rep(0,10),0.8,-0.4),lag.max=13)[-1],
     x = 1:13, type = "h", xlab="Lag k",
     ylab = expression(rho[k]), axes=FALSE)
points(y = ARMAacf(ma = c(-0.5,rep(0,10),0.8,-0.4),lag.max=13)[-1],
       x = 1:13, pch = 20)
abline(h = 0)
axis(1, at = 1:13, labels = c(1,NA,3,NA,5,NA,7,NA,9,NA,11,NA,13))
axis(2)
text(x = 7, y = 0.35, labels = expression(list(theta==0.5,Theta==-0.8)))


# Exhibit 10.4
plot(y = ARMAacf(ar = c(rep(0,11), 0.75), ma = 0.4,lag.max=61)[-1],
     x = 1:61, type = "h", xlab="Lag k",
     ylab = expression(rho[k]), axes = FALSE,
     ylim = c(-0.4,.8), xlim = c(0,61))
...
text(x = 40, y = 0.8, labels = expression(list(Phi==0.75,theta==-0.4)))

plot(y = ARMAacf(ar=c(rep(0,11), 0.75), ma = -0.4, lag.max = 61)[-1],
     x = 1:61, type = "h", xlab = "Lag k",
     ylab = expression(rho[k]), axes = FALSE, ylim = c(-0.4,.8))
points(y = ARMAacf(ar=c(rep(0,11), 0.75), ma=-0.4,lag.max=61)[c(1,11,12,13,
23,24,25,35,36,37,47,48,49,59,60,61)+1],
       x = c(1,11,12,13,23,24,25,35,36,37,47,48,49,59,60,61), pch = 20)
abline(h = 0)
...
CO[2]~~Levels), 
     ylab = expression(First~~Difference~~of~~CO[2])) 
...
# Exhibit 10.15
m2.co2 <- Arima(co2, order = c(0,1,2), 
                seasonal = list(order=c(0,1,1), period = 12))
m2.co2
