...
plot(ts(Nine11p * (-0.0949) + 
        filter(Nine11p, filter = 0.8139, method = "recursive", side = 1) * (-0.2715),
        frequency = 12, start = 1996),
     type = "h", ylab = "9/11 Effects")
abline(h = 0)
...
plot(y, type = "b") 
arrows(30, 6, 11, 9.8, length = 0.1, angle = 20)
text(34,6.2, "AO")
...
n <- 500
for (i in 1:B) {
  x <- cumsum(arima.sim(model = list(ma = 0.8), n = n))
  y <- cumsum(arima.sim(model = list(ma = 0.8), n = n))
  correlation.v <- c(correlation.v, ccf(x, y, lag.max=1, plot=FALSE)$acf[2])
  }

hist(correlation.v, prob = TRUE, xlab = expression(r[0](X,Y)))
...
plot(milk.electricity, yax.flip = TRUE) 
...
tsdiag(boardings.m2, tol = 0.15, gof.lag = 24)
